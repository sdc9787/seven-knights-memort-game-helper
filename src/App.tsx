...
const REFERENCE_BACK_BRIGHTNESS = 130;
const DEFAULT_RECT = { x: 42, y: 76, w: 514, h: 251, gapX: 8, gapY: 4, offsetX: 1, offsetY: -1, detectionW: 25, detectionH: 8 };
...
export default function App() {
  ...
  const detectionWRef = useRef(DEFAULT_RECT.detectionW);
  const detectionHRef = useRef(DEFAULT_RECT.detectionH);
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (card.image) {
        ...
        if (img.complete) {
          ...
          // 뒤집혔을 때(앞면)만 어둡게 처리 (brightness 0.4 상당)
          // 뒷면일 때는 원상태 유지
          if (card.isFlipped) {
  ...
  const getBrightnessFromPixels = (pixels: Uint8ClampedArray, rect: { x: number, y: number, w: number, h: number }, canvasW: number, canvasH: number, vOffX: number = 0, vOffY: number = 0, vDW: number = 25, vDH: number = 8) => {
    const centerX = Math.round(rect.x + rect.w / 2 + vOffX);
    const centerY = Math.round(rect.y + rect.h / 2 + vOffY);

    const startX = Math.max(0, centerX - Math.floor(vDW / 2));
    const startY = Math.max(0, centerY - Math.floor(vDH / 2));

    let brightnessSum = 0;
    let count = 0;
    for (let y = startY; y < startY + vDH && y < canvasH; y++) {
      for (let x = startX; x < startX + vDW && x < canvasW; x++) {
  ...
  const updateGridFromSelection = useCallback((sel: SelectionRect, gapX: number = 0, gapY: number = 0, offX: number = 0, offY: number = 0, dW: number = 25, dH: number = 8) => {
    ...
    // Use clientWidth/Height to exclude borders for accurate mapping
    ...
    // Scale detection size to match visual UI box
    detectionWRef.current = Math.round(dW * scaleX);
    detectionHRef.current = Math.round(dH * scaleY);
  ...
  const recalibrateBaseline = useCallback(() => {
    ...
    const vDW = detectionWRef.current;
    const vDH = detectionHRef.current;

    setCards(prev => prev.map(card => {
      if (!card.rect) return card;
      const baseline = getBrightnessFromPixels(pixels, card.rect, canvas.width, canvas.height, vOffsetXRef.current, vOffsetYRef.current, vDW, vDH);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    // Adjust for border offset (1px)
    const borderLeft = (rect.width - container.clientWidth) / 2;
    const borderTop = (rect.height - container.clientHeight) / 2;
  ...
  const handleMouseUp = () => {
    ...
    if (width > 20 && height > 20) {
      ...
      setManualRect({
        ...
        detectionW: manualRect.detectionW,
        detectionH: manualRect.detectionH
      });
      updateGridFromSelection(newSelection, manualRect.gapX, manualRect.gapY, manualRect.offsetX, manualRect.offsetY, manualRect.detectionW, manualRect.detectionH);
    }
  ...
  const applyManualRect = () => {
    ...
    setSelection(newSelection);
    updateGridFromSelection(newSelection, manualRect.gapX, manualRect.gapY, manualRect.offsetX, manualRect.offsetY, manualRect.detectionW, manualRect.detectionH);
    addLog(`수동 좌표 적용: X=${manualRect.x}, Y=${manualRect.y}, W=${manualRect.w}, H=${manualRect.h}, GapX=${manualRect.gapX}, GapY=${manualRect.gapY}, OffX=${manualRect.offsetX}, OffY=${manualRect.offsetY}, DW=${manualRect.detectionW}, DH=${manualRect.detectionH}`);
  };
  ...
  const processFrame = useCallback((time: number) => {
    ...
    for (let i = 0; i < currentCards.length; i++) {
      ...
      const brightness = getBrightnessFromPixels(pixels, card.rect, canvas.width, canvas.height, vOffsetXRef.current, vOffsetYRef.current, vDW, vDH);
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        videoRef.current.onloadedmetadata = () => {
          ...
          if (selection) {
            updateGridFromSelection(selection, manualRect.gapX, manualRect.gapY, manualRect.offsetX, manualRect.offsetY, manualRect.detectionW, manualRect.detectionH);
            ...
            updateGridFromSelection(defaultSel, manualRect.gapX, manualRect.gapY, manualRect.offsetX, manualRect.offsetY, manualRect.detectionW, manualRect.detectionH);
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-6 space-y-6">
          ...
          <section className="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-4 grid grid-cols-2 md:grid-cols-6 gap-4 shadow-inner">
            ...
            <div className="space-y-1 border-r border-zinc-800/50 pr-2">
              <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider">감지 크기</p>
              <p className="text-xs font-mono text-purple-400">{manualRect.detectionW}x{manualRect.detectionH}</p>
            </div>
          ...
          >
            <video ref={videoRef} className="w-full h-full object-fill pointer-events-none" muted playsInline />
                  ...
                  {Array.from({ length: 24 }).map((_, i) => (
                    <div key={i} className="border border-green-500/30 flex items-center justify-center relative">
                      ...
                      <div
                        ...
                        style={{
                          width: `${manualRect.detectionW}px`,
                          height: `${manualRect.detectionH}px`,
                  ...
                  <div className="p-4 pt-0 space-y-4 border-t border-zinc-800/50">
                    ...
                    <div className="space-y-4">
                            ...
                            <div key={field.key} className="space-y-1.5">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setSelection(newSelection);
                                  updateGridFromSelection(newSelection, next.gapX, next.gapY, next.offsetX, next.offsetY, next.detectionW, next.detectionH);
                                }}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-3 bg-zinc-950/30 rounded-xl border border-zinc-800/50">
                          {[
                            { label: 'X 오프셋', key: 'offsetX' },
                            { label: 'Y 오프셋', key: 'offsetY' },
                            { label: '감지 너비', key: 'detectionW' },
                            { label: '감지 높이', key: 'detectionH' },
                          ].map((field) => (
                            <div key={field.key} className="space-y-1.5">
                              <label className="text-[10px] text-zinc-500 font-medium">{field.label}</label>
                              <input
                  ...
                  {card.image && (
                    <>
                      <motion.img
                        ...
                        src={card.image}
                        alt={`Card ${idx + 1}`}
                        className={`w-full h-full object-cover transition-all duration-300 ${card.isFlipped ? 'brightness-[0.4]' : 'brightness-[1.0]'
                          }`}
